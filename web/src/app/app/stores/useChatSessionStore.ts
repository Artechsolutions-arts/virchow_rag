...
export const useCurrentMessageHistory = () => {
  ...
  return useMemo(() => {
    if (!messageTree) {
      console.log("[store] useCurrentMessageHistory: no messageTree");
      return [];
    }
    const chain = getLatestMessageChain(messageTree);
    console.log("[store] useCurrentMessageHistory: chain length=", chain.length, "types=", chain.map(m => m.type));
    return chain;
  }, [messageTree]);
};
