...
export default function AppPage({ firstMessage }: ChatPageProps) {
  ...
  // Zustand-driven session id — set immediately on submit, before URL updates
  const storeCurrentSessionId = useChatSessionStore(
    (state) => state.currentSessionId
  );
  // Effective session id: URL-driven (authoritative) or store-driven (for in-flight new sessions)
  const effectiveSessionId = currentChatSessionId || storeCurrentSessionId;
  const messageHistory = useCurrentMessageHistory();
  // True as soon as the first message lands in the store — used to mount ChatUI
  // before the URL reflects the new chatId (router.push is async).
