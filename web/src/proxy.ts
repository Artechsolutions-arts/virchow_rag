...
export const config = {
  matcher: [
    ...
    // API routes: inject Authorization header so backend rewrites receive the token
    "/api/:path*",
  ],
};
...
export async function proxy(request: NextRequest) {
  ...
  // For /api/* routes: inject Authorization header from cookie so the backend
  // receives the bearer token when the next.config.js rewrite proxies the request.
  if (pathname.startsWith("/api/")) {
    const token = request.cookies.get(FASTAPI_USERS_AUTH_COOKIE_NAME)?.value;
    if (token) {
