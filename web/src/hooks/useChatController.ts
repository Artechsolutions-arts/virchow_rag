...
interface MdChunksAskResponse {
  answer?: string;
  sources?: Array<{ doc_id: string; source_file: string; file_id?: string | null; url?: string | null }>;
  duration_ms?: number;
}
...
export default function useChatController({
  ...
  const handleNewSessionNaming = async (_chatSessionId: string) => {
    // Backend sets the chat title automatically from the first user message
    // (update_chat_title_if_empty in rag_pipeline.py). Give it 200ms then refresh.
  ...
  const upsertToCompleteMessageTree = ({
    ...
    console.log("[chat] updateSessionMessageTree session=", chatSessionId, "tree size=", newCompleteMessageTree.size);
  ...
  const onSubmit = useCallback(
    async ({
      ...
      // Read directly from the Zustand store to avoid stale closure — the closure variable
      // may lag behind if React hasn't re-rendered since the last streaming tick.
      const _treeSessionId = existingChatSessionId || useChatSessionStore.getState().currentSessionId;
      let currentMessageTreeLocal =
        (_treeSessionId
          ? useChatSessionStore.getState().sessions.get(_treeSessionId)?.messageTree
          : undefined) ||
        currentMessageTree ||
        new Map<number, Message>();
      ...
      console.log("[chat] onSubmit sessionId=", currChatSessionId, "isNew=", isNewSession);
      updateStatesWithNewSessionId(currChatSessionId);

      // Set chatState to "loading" BEFORE navigation so that if the navigation
      // triggers useChatSessionController's useEffect immediately, the live state
      // is "loading" (not "input"), which prevents initialSessionFetch from running
      // and clobbering the in-flight message stream.
      updateChatStateAction(currChatSessionId, "loading");
      ...
      console.log("[chat] upserting initial messages, frozenSessionId=", frozenSessionId);
      ...
      console.log("[chat] useMdChunksChat path, fetching answer for session=", frozenSessionId);
      if (useMdChunksChat) {
        ...
        try {
          ...
          // Simulate token-by-token streaming: reveal the answer in small chunks
          const CHARS_PER_TICK = 12;
          const TICK_MS = 20;
          await new Promise<void>((resolve) => {
            let charIndex = 0;
            const tick = () => {
              charIndex = Math.min(charIndex + CHARS_PER_TICK, mdChunksAnswer.length);
              const partial = mdChunksAnswer.slice(0, charIndex);
              currentMessageTreeLocal = upsertToCompleteMessageTree({
                messages: [
                  { ...initialUserNode, files },
                  {
                    ...initialAgentNode,
                    message: partial,
                    type: "assistant",
              ...
              if (charIndex < mdChunksAnswer.length) {
              ...
              },
              {
                ...
                message:
                  errMsg.includes("aborted") || errMsg.includes("signal")
                    ? "Answer timed out. The model is still processing — please refresh the page in a moment to see the response."
