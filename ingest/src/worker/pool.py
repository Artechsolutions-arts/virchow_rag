...
from src.config import MQ_QUEUE_PRIORITY, MQ_QUEUE_NORMAL, MQ_QUEUE_LARGE, MAX_RETRIES, cfg
...
# Local message buffer per feeder — unbounded so feeders never block on submit.
_INBOX_MAXSIZE = 0
...
class PDFWorker:
    ...
    def _handle(self, body: bytes, ch, method):
        ...
        try:
            ...
            if not fpath.exists():
                # Native runner: job path uses Docker /app/uploads/ prefix;
                # remap to actual host upload dir using just the filename.
                fpath = cfg.upload_dir / Path(job.file_path).name
            if not fpath.exists():
