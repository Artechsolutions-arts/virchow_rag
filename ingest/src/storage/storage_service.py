...
class StorageService:
    ...
    async def store_pdf_from_path(
        ...
        Pass dest_filename to override the on-disk name (e.g. strip UUID prefix)."""
        local_path = Path(local_path)
        name = dest_filename if dest_filename else local_path.name
        key = raw_pdf_key(name)
    ...
    async def delete_job_artefacts(self, job_id: str) -> int:
        """Remove objects associated with job_id (chunks only; raw/processed are per-filename now)."""
        prefixes = [f"chunks/{job_id}/"]
        deleted = 0
    ...
    def pdf_url(self, job_id: str, filename: str) -> str:
        return self._client.public_url(raw_pdf_key(filename))

    def processed_url(self, job_id: str, filename: str) -> str:
        return self._client.public_url(processed_key(filename))
    ...
    async def list_job_files(self, job_id: str) -> dict[str, list[dict]]:
        raw = await self._client.list_files("raw/")
        processed = await self._client.list_files("processed/")
