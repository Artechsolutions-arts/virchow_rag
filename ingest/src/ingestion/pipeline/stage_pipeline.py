...
┌─ Stage 5 ──────────────────▼───────────────────────────────┐
│  ColPali Visual Embedder  · 1 CPU thread                   │
│  Detects image-heavy/handwritten pages via base64 & word   │
│  count heuristic, renders them with PyMuPDF, embeds with   │
│  vidore/colpali-v1.2 (128-dim), stores in DB immediately   │
│  if doc_id is known (user uploads) else defers to chunker  │
└────────────────────────────┬───────────────────────────────┘
                             │ post_colpali_q  (maxsize=50)
┌─ Stage 6 ──────────────────▼───────────────────────────────┐
    ...
    def _safe_load_hf_model(self):
        ...
        try:
            with self._load_lock, contextlib.redirect_stdout(io.StringIO()):
                ...
                # On CPU: force every parameter AND buffer to float32.
                # from_pretrained respects torch_dtype for parameters but leaves
                # registered buffers (e.g. vision encoder positional biases) in
                # their saved dtype (BF16), causing
                # "Input type BFloat16 and bias type float" errors at inference.
                if device == "cpu":
...
# ── Stage thread counts (env-configurable for CPU/GPU split tuning) ──────────
# Docker defaults: conservative values (CPU-only inside container).
# Native macOS with MPS: use run_native.sh which sets much higher values.
import os as _os_stage
N_PREPROCESS = int(_os_stage.environ.get("N_PREPROCESS_WORKERS", "6"))
N_OCR        = int(_os_stage.environ.get("N_OCR_WORKERS",        "3"))
N_CHUNK      = int(_os_stage.environ.get("N_CHUNK_WORKERS",      "4"))
N_STORE      = int(_os_stage.environ.get("N_STORE_WORKERS",      "4"))
EMBED_BATCH  = int(cfg.embedding_batch)   # default 128, overridden by EMBEDDING_BATCH_SIZE
...
# Pages with fewer than this many words are treated as image-heavy and queued
# for ColPali visual embedding. Override with env var COLPALI_WORD_THRESHOLD.
import os as _os
import re as _re
COLPALI_WORD_THRESHOLD = int(_os.environ.get("COLPALI_WORD_THRESHOLD", "80"))

# DotsOCR embeds base64-encoded image data as plain text (e.g. figure captions
# become "![img](data:image/png;base64,AAAA...==)"). Strip those tokens before
# word counting so the count reflects actual readable text, not binary noise.
# Additionally, any page that contains base64 data has an embedded image/figure
# and should always be flagged for ColPali regardless of word count.
_BASE64_RE = _re.compile(r'[A-Za-z0-9+/]{60,}={0,2}')


def _colpali_detect(page_md: str) -> bool:
    """Return True if the page should get a ColPali visual embedding."""
    has_embedded_image = bool(_BASE64_RE.search(page_md))
    if has_embedded_image:
        return True
    cleaned = _BASE64_RE.sub(' ', page_md)
    return len(cleaned.split()) < COLPALI_WORD_THRESHOLD
...
class StagePipeline:
    ...
    def __init__(self, rsm, rbac, storage=None):
        ...
        # ── Inter-stage queues ────────────────────────────────────────────────
        # All queues are unbounded (maxsize=0) so any number of files can be
        # submitted without blocking feeders. Backpressure comes from worker
        # throughput, not queue caps.
        # _doc_q is a PriorityQueue: (file_size_kb, DocJob) — smaller files first.
        self._doc_q           = PriorityQueue(maxsize=0)
        self._page_q          = Queue(maxsize=0)   # PageJobs (PIL images)
        self._markdown_q      = Queue(maxsize=0)   # PageMarkdown (text only)
        self._assembled_q     = Queue(maxsize=0)   # AssembledDoc → ColPali input
        self._post_colpali_q  = Queue(maxsize=0)   # AssembledDoc → Chunk input (after ColPali)
        self._chunk_q         = Queue(maxsize=0)   # ChunkItem
        self._store_q         = Queue(maxsize=0)   # EmbeddedItem
        ...
        # Pre-warm the dots_ocr import in the main thread before any worker
        # threads start.  trust_remote_code=True in sentence-transformers calls
        # importlib.invalidate_caches(), which briefly breaks concurrent imports.
        # Pre-importing here puts dots_ocr in sys.modules so OCR threads skip
        # the filesystem search entirely and avoid that race window.
        try:
            import dots_ocr as _dots_ocr_warmup  # noqa: F401
            del _dots_ocr_warmup
            logger.info("[OCR] dots_ocr pre-warmed into sys.modules")
        except ImportError as _e:
            logger.warning("[OCR] dots_ocr pre-warm failed: %s", _e)
    ...
    def start(self):
        spawns = [
            ...
            *[self._make(f"ocr-{i}",      self._ocr_worker)          for i in range(N_OCR)],
            ...
            self._make("colpali",         self._colpali_worker),
        ...
        logger.info(
            ...
            "(preprocess=%d ocr=%d assembler=1 colpali=1 chunk=%d embed=1 store=%d)",
    ...
    def stop(self, timeout: float = 30.0):
        ...
        # _doc_q is a PriorityQueue — its sentinel is a priority tuple.
        _doc_stop = (float("-inf"), _STOP)
        for _ in range(len(self._threads) * 2):
            for q in (self._page_q, self._markdown_q,
                      self._assembled_q, self._post_colpali_q,
                      self._chunk_q, self._store_q):
                try:
                    q.put_nowait(_STOP)
                except Full:
                    pass
            try:
                self._doc_q.put_nowait(_doc_stop)
            except Full:
                pass
    ...
    def submit(self, doc_job: DocJob):
        """Submit a document — smallest files are processed first."""
        self._put(self._doc_q, (doc_job.file_size_kb, doc_job))
    ...
    def _preprocess_worker(self):
        ...
        while not self._shutdown.is_set():
            ...
            if item is None:
                break
            # _doc_q stores (priority, DocJob) tuples; unpack.
            _, doc = item if isinstance(item, tuple) else (0, item)
            doc: DocJob = doc
            try:
                ...
                if self.rsm:
                    ...
                    if existing_id:
                        logger.info("[Preprocess] '%s' already processed (cached) — skipped",
                                    doc.filename)
                        self._mark_pending_completed_on_dedup(doc)
                ...
                if self.rbac and doc.upload_id:
                    try:
                        _pid = self.rbac.find_document_by_upload_id(doc.upload_id)
                        if _pid:
                            doc.db_doc_id = _pid
                            self.rbac.update_document_stage(
                                _pid, "preprocessing",
                                processing_started_at=_started,
                            )
                    except Exception as _e:
                        logger.debug("[Preprocess] stage-db-update skipped: %s", _e)
                ...
                self._fail(doc.file_id, doc.session_id, str(e), file_path=doc.file_path)
    ...
    def _ocr_worker(self):
        ...
        while not self._shutdown.is_set():
            ...
            # Flush MPS allocator cache after every page so the GPU memory
            # pool doesn't grow unbounded across thousands of documents.
            try:
        ...
        while not self._shutdown.is_set():
            ...
            if fid not in in_flight:
                in_flight[fid] = {
                    ...
                    "db_doc_id":  pm.doc_job.db_doc_id,
                }
                self._update_stage(fid, pm.session_id, "assembling", 50)

            in_flight[fid]["pages"][pm.page_idx] = pm.markdown or ""

            # Update OCR page progress in DB so the UI shows "OCR — X of Y"
            _db_did = in_flight[fid]["db_doc_id"]
            if self.rbac and _db_did:
                try:
                    self.rbac.update_document_stage(
                        _db_did, "ocr",
                        ocr_current_page=len(in_flight[fid]["pages"]),
                        ocr_total_pages=in_flight[fid]["total"],
                    )
                except Exception as _e:
                    logger.debug("[Assembler] ocr-progress skipped: %s", _e)
            ...
            # Detect image-heavy pages for ColPali: pages with embedded images
            # (base64 data in OCR output) or below word threshold after stripping noise.
            visual_page_nums = [
                i for i, page_md in enumerate(ordered)
                if _colpali_detect(page_md)
            ]
            if visual_page_nums:
                logger.debug("[Assembler] '%s' — %d visual pages flagged for ColPali: %s",
                             doc_job.filename, len(visual_page_nums), visual_page_nums[:10])
            ...
            if not clean_md.strip():
                ...
                self._fail(fid, session, "No text extracted from document",
                           file_path=doc_job.file_path)
                # Mark the pending documents record as failed so it shows in the UI.
                if self.rbac and doc_job.upload_id and doc_job.upload_type == "user":
                    try:
                        existing_id = self.rbac.find_document_by_upload_id(doc_job.upload_id)
                        if existing_id:
                            self.rbac.mark_document_failed(existing_id)
                    except Exception as _e:
                        logger.debug("[Assembler] mark_document_failed skipped: %s", _e)
                continue
            ...
            self._put(self._assembled_q, AssembledDoc(
                ...
                doc_job=doc_job,
                visual_page_nums=visual_page_nums,
            ))
    ...
    def _chunk_worker(self):
        """
        Stage 6 — reads from _post_colpali_q (after ColPali has run).
        Creates a Postgres document record then emits ChunkItems.
        Also stores any deferred ColPali vectors (admin uploads) now that
        the doc_id is available.
        Dedup is handled upstream in the preprocess worker.
        """
        while not self._shutdown.is_set():
            item = self._get(self._post_colpali_q)
            if item is None:
                break
            ...
            try:
                ...
                if self.rbac:
                    ...
                    try:
                        _file_size = Path(adoc.doc_job.file_path).stat().st_size
                    except OSError:
                        _file_size = 0
                    try:
                        # If upload endpoint created a pending record, update it
                        # instead of inserting a duplicate.
                        existing_id = self.rbac.find_document_by_upload_id(u_id) if u_id else None
                        if existing_id:
                ...
                if self.rbac and doc_id:
                    try:
                        self.rbac.update_document_extraction(
                            doc_id, {**extraction, "ocr_quality": doc_quality,
                                     "ocr_quality_score": doc_quality,
                                     "extracted_text": adoc.markdown[:65535]})
                    except Exception as e:
                    ...
                    # Store any deferred ColPali vectors from admin uploads
                    # (user uploads were stored immediately in the colpali_worker)
                    if adoc.colpali_vectors:
                        stored = 0
                        for pnum, vec in adoc.colpali_vectors.items():
                            try:
                                ...
                                logger.warning(
                                    "[ColPali] deferred store failed page %d '%s': %s",
                                    pnum, adoc.doc_job.filename, e)
                        if stored:
                            logger.info("[ColPali] '%s' — deferred %d embeddings stored",
                                        adoc.doc_job.filename, stored)
                ...
                if self.rbac and doc_id:
                    try:
                        self.rbac.update_document_stage(doc_id, "chunking")
                    except Exception as _e:
                        logger.debug("[Chunk] stage-update skipped: %s", _e)
    ...
    def _flush_embed_batch(self, batch: list[ChunkItem], embedder: MxbaiEmbedder):
        ...
        for chunk_item, vector in zip(batch, vectors):
            if chunk_item.file_id not in seen_files:
                ...
                if self.rbac:
                    with self._chunk_lock:
                        _ctr = self._chunk_counter.get(chunk_item.file_id, {})
                        _db_stage = _ctr.get("db_stage")
                        _doc_id   = _ctr.get("doc_id")
                    if _db_stage != "embedding" and _doc_id:
                        try:
                            self.rbac.update_document_stage(_doc_id, "embedding")
                            with self._chunk_lock:
                                if chunk_item.file_id in self._chunk_counter:
                                    self._chunk_counter[chunk_item.file_id]["db_stage"] = "embedding"
                        except Exception as _e:
                            logger.debug("[Embed] stage-update skipped: %s", _e)
    ...
    def _store_worker(self):
        ...
        while not self._shutdown.is_set():
            ...
            if first_chunk:
                ...
                if self.rbac and doc_id:
                    try:
                        self.rbac.update_document_stage(doc_id, "storing")
                    except Exception as _e:
                        logger.debug("[Store] stage-update skipped: %s", _e)
            ...
            if done:
                ...
                # ColPali already ran before this stage — always delete the file now.
    ...
    # ── Stage 5: ColPali Visual Embedder ─────────────────────────────────────

    def _colpali_worker(self):
        """
        Stage 5 — runs BETWEEN the assembler and the chunk worker.

        Reads AssembledDocs from _assembled_q.  For documents with visual pages
        (base64-embedded images, low word-count pages, or handwriting) it:
          1. Renders those pages to PIL images via PyMuPDF.
          2. Embeds them with vidore/colpali-v1.2 → 128-dim L2-normalised vectors.
          3. Stores vectors immediately when doc_id is available (user uploads
             where the upload API pre-created a pending documents row).
          4. For admin uploads (no pre-created row) the vectors are attached to
             the AssembledDoc so the chunk_worker stores them after creating the
             document record.

        Passes each AssembledDoc to _post_colpali_q regardless of outcome so the
        text pipeline is never blocked.  File deletion is handled by the
        store_worker (ColPali no longer owns the file).
        """
        colpali = None
        ...
        while not self._shutdown.is_set():
            ...
            try:
                if adoc.visual_page_nums:
                    # Lazy-load ColPali model on first visual document
                    if colpali is None:
                        try:
                            ...
                            logger.error("[ColPali] Failed to load ColPaliEmbedder: %s", e)

                    if colpali is not None:
                        import fitz
                        from PIL import Image
                        import io

                        logger.info("[ColPali] Processing '%s' — %d visual pages: %s",
                                    adoc.doc_job.filename, len(adoc.visual_page_nums),
                                    adoc.visual_page_nums[:10])

                        fitz_doc = fitz.open(adoc.doc_job.file_path)
                        page_images: list = []
                        valid_page_nums: list = []

                        for pg in adoc.visual_page_nums:
                            if pg >= len(fitz_doc):
                                continue
                            try:
                                mat = fitz.Matrix(2.0, 2.0)
                                pix = fitz_doc[pg].get_pixmap(matrix=mat)
                                img = Image.open(io.BytesIO(pix.tobytes("png"))).convert("RGB")
                                page_images.append(img)
                                valid_page_nums.append(pg)
                            except Exception as pe:
                                logger.warning("[ColPali] Page %d render failed for '%s': %s",
                                               pg, adoc.doc_job.filename, pe)
                        fitz_doc.close()

                        if page_images:
                            BATCH = 4
                            all_vecs: dict[int, list] = {}
                            for b_start in range(0, len(page_images), BATCH):
                                batch_imgs = page_images[b_start:b_start + BATCH]
                                batch_nums = valid_page_nums[b_start:b_start + BATCH]
                                try:
                                    vecs = colpali.embed_pages(batch_imgs)
                                except Exception as e:
                                    logger.error("[ColPali] embed_pages failed: %s", e)
                                    vecs = [[0.0] * 128] * len(batch_imgs)
                                for pnum, vec in zip(batch_nums, vecs):
                                    all_vecs[pnum] = vec

                            # Try to store immediately if doc_id is already known
                            doc_id = adoc.doc_job.db_doc_id
                            if not doc_id and self.rbac and adoc.doc_job.upload_id and \
                                    adoc.doc_job.upload_type == "user":
                                try:
                                    doc_id = self.rbac.find_document_by_upload_id(
                                        adoc.doc_job.upload_id)
                                except Exception:
                                    pass

                            if doc_id and self.rbac:
                                stored = 0
                                for pnum, vec in all_vecs.items():
                                    try:
                                        self.rbac.store_colpali_embedding(
                                            doc_id=doc_id,
                                            dept_id=adoc.doc_job.dept_id,
                                        ...
                                        logger.warning(
                                            "[ColPali] store failed page %d of '%s': %s",
                                            pnum, adoc.doc_job.filename, e)
                                logger.info("[ColPali] '%s' — stored %d/%d embeddings",
                                            adoc.doc_job.filename, stored, len(all_vecs))
                            else:
                                # doc_id not yet available (admin upload without pre-created
                                # document row) — defer to chunk_worker via colpali_vectors
                                adoc.colpali_vectors = all_vecs
                                logger.info("[ColPali] '%s' — %d vectors deferred to chunk_worker",
                                            adoc.doc_job.filename, len(all_vecs))

            except Exception as e:
                logger.error("[ColPali] '%s' failed: %s",
                             adoc.doc_job.filename, e, exc_info=True)
            finally:
                # Always pass downstream regardless of ColPali success/failure
                self._put(self._post_colpali_q, adoc)

    # ── Helpers ───────────────────────────────────────────────────────────────

    def _get(self, q, timeout: float = 2.0):
        ...
        while not self._shutdown.is_set():
            try:
                ...
                if item is _STOP:
                    return None
                # PriorityQueue sentinel: (float('-inf'), _STOP)
                if isinstance(item, tuple) and len(item) == 2 and item[1] is _STOP:
                    return None
                return item
            except Empty:
                continue
    ...
    def _fail(self, file_id: str, session_id: str, error: str, file_path: str = None):
        ...
        if file_path:
            try:
                ...
                if p.exists():
                    p.unlink()
                    logger.debug("[Fail] Deleted upload file after failure: %s", file_path)
            except Exception as e:
                logger.warning("[Fail] Could not delete '%s': %s", file_path, e)

    def _mark_pending_completed_on_dedup(self, doc: "DocJob"):
        """If a pending DB record was created for this upload, mark it completed
        so the UI doesn't show it stuck as IN PROGRESS after a dedup skip."""
        if not self.rbac:
            return
        ...
        if not u_id:
            return
        try:
            pending_id = self.rbac.find_document_by_upload_id(u_id)
            if pending_id:
                self.rbac.update_document_status(pending_id, "completed")
        except Exception as e:
            logger.debug("[Preprocess] dedup pending-mark failed: %s", e)
