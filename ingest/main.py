...
async def _run_worker_only():
    ...
    # 7. Recover stuck jobs from previous crash ────────────────────────────────
    # Disabled: supervisor handles all re-queuing via stall detection.
    # Per-worker recovery causes N×workers duplicate messages on restart.
    if False and _mq_conn and _pool_pg:
        try:
            from src.database.rabbitmq_broker import publish_job
            from src.models.schemas import BatchSession, FileProgress, JobPayload
            import uuid as _uuid, time as _time, pathlib as _pathlib

            rbac_recovery = RBACManager(_pool_pg)
            stuck = rbac_recovery.find_stuck_documents(stale_minutes=3)
            recovered = 0
            for doc in stuck:
                fpath = _pathlib.Path(doc["file_path"])
                # If the stored path uses the Docker-container prefix (/app/uploads/)
                # but we're running natively, remap to the real uploads directory.
                if not fpath.exists() and "/app/uploads/" in doc["file_path"]:
                    fname = _pathlib.Path(doc["file_path"]).name
                    fpath = cfg.upload_dir / fname
                if not fpath.exists():
                    logger.warning("[Recovery] Stuck doc '%s' — file missing, skipping",
                                   doc["file_name"])
                    continue
                try:
                    rbac_recovery.reset_document_for_retry(doc["doc_id"])
                    ...
                    if _rsm:
                        session = BatchSession(
                            session_id=session_id, total=1,
                            user_id=doc["user_id"], dept_id=doc["dept_id"],
                            upload_type=upload_type,
                        )
                        _rsm.create_session(session)
                        fp = FileProgress(
                        ...
                        _rsm.register_file(session_id, fp)

                    job = JobPayload(
                        session_id=session_id, file_id=file_id,
                        filename=doc["file_name"], file_path=str(fpath),
                        file_size_kb=fpath.stat().st_size / 1024,
                        user_id=doc["user_id"], dept_id=doc["dept_id"],
                        upload_type=upload_type, upload_id=upload_id,
                    )
                    publish_job(job)
                    recovered += 1
                    logger.info("[Recovery] Re-queued stuck doc '%s' (doc_id=%s)",
                                doc["file_name"], doc["doc_id"])
                except Exception as re:
                    logger.warning("[Recovery] Failed to re-queue '%s': %s",
                                   doc["file_name"], re)
            if stuck:
                logger.info("[Recovery] Found %d stuck doc(s), re-queued %d",
                            len(stuck), recovered)
        except Exception as e:
            logger.warning("[Recovery] Stuck-job scan failed: %s", e)
