...
class DotsOCRParser:
    ...
    def __init__(self, 
        ...
        # Set weights path: prioritize provided model_name if it's a valid path
        if os.path.exists(self.model_name):
            self.model_path = os.path.abspath(self.model_name)
        else:
            # Fallback to default
            self.model_path = os.path.abspath("./weights/DotsOCR")
    ...
    def _load_hf_model(self):
        ...
        device = get_device()
        # Weights are BF16. On GPU/MPS use BF16; on CPU force float32 since
        # BF16 convolutions require AVX512-BF16 which most CPUs lack.
        ...
        # On CPU: force every parameter AND buffer to float32 to avoid BF16 op
        # failures. .float() converts parameters but leaves registered buffers
        # (e.g. vision encoder's patch-embed positional bias) as BF16.
        if device == "cpu":
    ...
    def _inference_with_hf(self, image, prompt):
        ...
        MAX_PX = 672 * 672      # pixel budget (~0.45 M) — optimised for speed
        ...
        # On CPU, force float32 to avoid BF16 op failures (requires AVX512-BF16).
        # On GPU/MPS, cast to model's native dtype (usually BF16).
        target_dtype = torch.float32 if device == "cpu" else self.model.dtype
        ...
        # Free KV cache and input tensors from MPS immediately after generation.
